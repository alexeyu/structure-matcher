...
- **json** — depends on `core` + Jackson (and `implementation`-depends on `report` for canonical paths). Two distinct formats: (1) `Json.mapper()` returns a configured `ObjectMapper` that serializes a `FeedbackNode` tree to a nested, property-keyed JSON — a **human-readable rendering** (custom serializer for composite nodes; a mixin hides internal fields). (2) `FeedbackArchives` is the **stable, versioned persistence format**: a flat `FeedbackArchive` `{schemaVersion, matched, brokenLeaves:[{path, expectation, value}]}` built on `FeedbackQuery.brokenLeaves`, with `toJson`/`archive`/`fromJson` and `CURRENT_SCHEMA_VERSION` (reader rejects unknown versions, ignores unknown fields). A whole **batch** persists to one document as **JSON Lines** — `toJsonLines(feedbacks)`/`writeLines(archives)` write one compact archive per line (single-document `toJson` output stays indented; the batch form uses a compact `ObjectWriter`), `fromJsonLines` reads them back (skips blank lines, validates each line's version like `fromJson`) — so a persisted batch reloads and rolls up via `FeedbackAggregator.addBrokenPaths` with no report type in `json`'s API. Treat the archive shape as an API; the nested rendering is for reading, not persistence.
- **report** — depends on `core` (zero extra runtime deps). Consumes the `FeedbackNode` tree to produce batch-level insight: flatten broken paths and aggregate N comparisons into a `FeedbackSummary`. Kept separate from core (like `json`) so the matching core stays a pure "produce feedback" library and the analysis surface can grow.
- **examples** — runnable end-to-end usage (the bookstore XML-vs-JSON scenario from the README); depends on `core` + `json` + `report`. Model POJOs and integration tests live together under `examples/.../bookstore`: `ResponseMatchingTest` (single compare, custom matchers, typed vs string paths) and `BatchReportTest` (the batch scenario — aggregate N comparisons into a `FeedbackSummary`, query one with `FeedbackQuery`, and persist+reload via `FeedbackArchives`).
...
The `report` module (`nl.alexeyu.structmatcher.report`, depends on `core` only) turns feedback into batch-level insight (Phase 3). `FeedbackPaths.brokenPaths(node)` flattens a tree into registration-style paths (drops the root class name, joins structure children with `.`, keeps collection brackets without repeating the parent name: `Sub.Bool`, `Strings[0]`, `Books[0].Authors[0].FirstName`); `FeedbackPaths.toFieldPath` collapses `[index]`/`[key]`/`[element]` to `[]`. `FeedbackAggregator` (`add`/`addAll`/`summary`, or one-shot static `summarize`) accumulates many comparisons into a `FeedbackSummary`: total / matched / mismatched, `mismatchRate`, per-field failure counts and rates (a field is counted at most once per comparison, ordered most-failing first), and `topMismatchingFields`. `add(FeedbackNode)` delegates to `addBrokenPaths(Collection<String>)`, the reload entry point that aggregates a comparison straight from its stored canonical paths (e.g. `json`'s `FeedbackArchive.brokenPaths()`) — so a persisted batch rolls up without rebuilding the tree, and live/reloaded inputs share one code path (report stays core-only; the bridge is plain strings). For single-tree inspection, `FeedbackQuery` returns `BrokenLeaf` records (canonical path + the `ExpectationBroken` leaf, so callers get both *where* and *why* without re-walking): `brokenLeaves(tree)`, `find(tree, predicate)`, and `mismatchesUnder(tree, prefix)` (segment-aware prefix match). The tree traversal is centralized in `FeedbackPaths.brokenLeaves` (package-private); `brokenPaths` maps over it. Zero extra runtime deps; aggregation is not thread-safe (aggregate from one thread), `FeedbackQuery`/`FeedbackPaths` are pure functions. Its end-to-end tests use a small local model (`SampleStructure`) because core's test fixtures aren't visible across the module boundary.
